class Car {
    ...
    constructor(initRegistration, initColor, initWeight, initPower, initTankCapacity, initNbPlaces) {
        ...
        if (this.registrationIsValid(initRegistration)) {
            ...
            throw new Error("Format de la plaque d'immatriculation invalide, veuillez entrer une plaque au format AB-123-CD");
        }
        /** @return {string} */
    ...
    weightIsValid(weight) {
        return Number.isInteger(weight) && weight > 0 && weight <= 1500;
    }
     ...
     */
    powerIsValid(power) {
        return Number.isInteger(power) && power > 0 && power <= 900;
    }
    ...
    tankCapacityIsValid(tankCapacity) {
        return typeof tankCapacity === 'number' && tankCapacity > 0 && tankCapacity <= 60;
    }
    ...
    nbPlacesIsValid(nbPlaces) {
        return Number.isInteger(nbPlaces) && nbPlaces >= 2 && nbPlaces <= 7;
    }
    ...
    levelFuelIsValid(levelFuel) {
        return typeof levelFuel === 'number';
    }
